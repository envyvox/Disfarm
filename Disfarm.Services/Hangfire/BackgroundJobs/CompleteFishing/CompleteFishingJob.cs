...
namespace Disfarm.Services.Hangfire.BackgroundJobs.CompleteFishing
{
    public class CompleteFishingJob : ICompleteFishingJob
    {
        ...
        public async Task Execute(ulong guildId, ulong channelId, long userId, uint cubeDrop)
        {
            ...
            var fish = await _mediator.Send(new GetRandomFishWithParamsQuery(rarity, state.WeatherToday, timesDay, state.CurrentSeason));
            var success = await _mediator.Send(new CheckFishingSuccessQuery(fish.Rarity, cubeDrop));
            var fishingXp = await _mediator.Send(new GetWorldPropertyValueQuery(WorldProperty.XpFishing));

            await _mediator.Send(new UpdateUserCommand(user with { Location = Location.Neutral }));
            await _mediator.Send(new AddXpToUserCommand(user.Id, fishingXp));
            ...
            await _mediator.Send(new SendEmbedToUserCommand(
                socketUser.Id, embed, GuildId: socketUser.Guild.Id, ChannelId: channelId));
        }
    }
}